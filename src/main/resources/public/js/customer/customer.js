layui.use(['table','layer',"form"],function(){
    ...
    table.on("toolbar(customers)",function (data){
        if(data.event=="add"){
            openAddOrUpdateCustomer();
        }else if(data.event=="link"){

        }else if(data.event=="recode"){

        }else if(data.event=="order"){
            var check = table.checkStatus(data.config.id);
            openCustomerOrder(check.data);
        }
    });
    ...
    function openCustomerOrder(data){
        ...
        var title = "订单查看";
        var url = ctx + "/customer/toCustomerOrderPager?id="+data[0].id;
        layui.layer.open({
            title:title,
            type:2,
            area:["700px","500px"],
    ...
    table.on('tool(customers)',function (data){
        if(data.event=="edit"){
            ...
            openAddOrUpdateCustomer(data.data.id)
        }else if(data.event=="del"){
            deleteCustomer(data.data.id);
        }
    });

    function openAddOrUpdateCustomer(id){
        var title = "<h1>客户信息-添加</h1>";
        var url = ctx + "/customer/toAddOrUpdateCustomerPager"
        if(id != null){
            title = "<h1>客户信息-修改</h1>";
            url+="?id="+id;
        }
    ...
    function deleteCustomer(id) {
        //     删除资源
        layer.confirm("你确定要删除选中的客户吗???",{icon:3,title:"客户管理"},function (index) {
            layer.close(index);
            $.ajax({
                type: "put",
